...
function inferCodeFromMessage(message: string): UnifiedErrorCode | null {
  ...
  if (containsAny(message, ['sensitive', 'unsafe', 'safety', 'blocked', 'prohibited', 'policy_violation', 'moderation', 'harm', 'celebrity', 'likenesses', 'rai', '敏感', '违规', '不当', '安全策略', '内容过滤']) && !containsAny(message, ['case-sensitive', 'case sensitive'])) return 'SENSITIVE_CONTENT'
