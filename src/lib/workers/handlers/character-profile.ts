...
async function handleConfirmProfile(
  ...
  const fallbackModel = readText(payload.model) || readText(payload.analysisModel)
  const project = await resolveProjectModel(job.data.projectId, fallbackModel || undefined)
